...
around the corner, and the shape of every room they have not entered yet. LineOfSight extends the
...
3. Right-click the block and choose **LoS**. Right-clicking only works while the GM
...
block you picked is now the board's LineOfSight switch:
...
- **Invisible to players.** GM blocks live on TaleSpire's GM-only render layer, which player
  clients never draw.
- **A single switch.** Choosing **LoS** on a different block moves the setting there.

Choosing **LoS** again turns it off and hands the block back to TaleSpire as an ordinary
...
| Button | Effect |
| --- | --- |
| **LoS** | Whether this board hides out-of-sight terrain at all |
...
**Ctrl+H** toggles hiding by hand, for a board with no LineOfSight block on it — handy for solo use
...
The explored map **survives quitting the game**. It is written to `BepInEx/LineOfSight/<board>.fog`,
...
`BepInEx/config/org.talespire.plugins.lineofsight.cfg`, written on first run. BepInEx does not
...
TaleSpire computes line of sight on the GPU to draw fog of war for creatures. This plugin reads
