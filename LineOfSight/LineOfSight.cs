    ...
    public class LineOfSightPlugin : DependencyUnityPlugin<LineOfSightPlugin>
    {
        ...
        protected override void OnAwake()
        {
            ...
            _gmBlockPlaceKey = Config.Bind("Diagnostics", "GmBlockPlace",
                KeyboardShortcut.Empty,
                "Place or remove an LineOfSight GM block at the selected creature, bypassing the radial "
            ...
            Logger.LogInfo($"[LineOfSight] Plugin loaded. Toggle={_toggleTrackingKey.Value} StaticHV={_staticHideVolumeKey.Value} AutoStart={_autoStartTracking.Value}");
        }
        ...
        private void SetupFogMaskReflection()
        {
            try
            {
                ...
                if (fogTaskType == null)
                {
                    Logger.LogWarning("[LineOfSight] ComputeFogUpdateMaskTask nested type not found");
                    return;
                }
                ...
                Logger.LogInfo($"[LineOfSight] ComputeFogUpdateMaskTask: _zone={_fmtZoneField != null} _updateMask={_fmtUpdateMaskField != null} _viewMap={_fmtViewMapField != null} UpdateProcess={updateProcessMethod != null}");

                if (updateProcessMethod != null)
                {
                    var prefix = AccessTools.Method(typeof(LineOfSightPlugin), nameof(FogTaskUpdateProcessPrefix));
                    var postfix = AccessTools.Method(typeof(LineOfSightPlugin), nameof(FogTaskUpdateProcessPostfix));
                    _harmony.Patch(updateProcessMethod,
                        prefix: new HarmonyMethod(prefix),
                        postfix: new HarmonyMethod(postfix));
                    Logger.LogInfo("[LineOfSight] Patched ComputeFogUpdateMaskTask.UpdateProcess (prefix+postfix)");
                }
        ...
        private static void LogMaskGrid(NativeArray<ushort> mask, Zone zone)
        {
            ...
            _log?.LogInfo($"[LineOfSight] MaskGrid zone={zone.Coord} origin={zone.Origin} (row=z, col=x, char=fogged y count)");
            for (int z = 0; z < ZONE_CELLS; z++)
            {
                ...
                _log?.LogInfo($"[LineOfSight] MaskGrid z{z,2}: {line}");
            }
        }
        ...
        private static void FogTaskUpdateProcessPostfix(object __instance, bool __result)
        {
            ...
            try
            {
                ...
                if (diagIdx <= 8)
                {
                    ...
                    _log?.LogInfo($"[LineOfSight] Diag#{diagIdx} zone={zone.Coord} origin={zone.Origin} {vmInfo}");
                    _log?.LogInfo($"[LineOfSight] Diag#{diagIdx} mask[0..7]={sb.ToString().TrimEnd()} dist={{0xFFFF={ffCount} zero={zeroCount} other={otherCount}}}");

                    if (diagIdx == 1) LogMaskGrid(mask, zone);
                }

                if (diagIdx <= 8)
                    _log?.LogInfo($"[LineOfSight] FogTask done: zone={zone.Coord} maskLen={mask.Length} (0xFFFF={ffCount} zero={zeroCount} other={otherCount})");
        ...
        protected override void OnDestroyed()
        {
            ...
            Logger.LogInfo("[LineOfSight] Plugin unloaded.");
        }
        ...
        private static bool TryGetSelectedCreature(out CreatureGuid creatureId)
        {
            ...
            {
                _log?.LogWarning("[LineOfSight] No creature selected.");
                return false;
            }
            return true;
        }
        ...
        private static bool TryGetCreatureWorldPos(CreatureGuid creatureId, out Vector3 worldPos)
        {
            ...
            {
                _log?.LogWarning("[LineOfSight] Could not find creature asset.");
                return false;
            }
            try
            {
            ...
            {
                _log?.LogWarning($"[LineOfSight] asset.Transform.position failed: {ex.Message}");
        ...
        private static bool TryFindZoneForWorldPos(Vector3 worldPos, out Zone zone)
        {
            ...
            if (board == null)
            {
                _log?.LogWarning("[LineOfSight] BoardSessionManager.Board is null.");
                return false;
            }
            ...
            {
                _log?.LogWarning($"[LineOfSight] No zone found at coord {coord} (world pos {worldPos}).");
                return false;
            }
            return true;
        }
        ...
        private void PollBoardChanges()
        {
            ...
            Logger.LogInfo($"[LineOfSight] [LoS] Board changed (event #{_lastBoardEventCounter}) — refreshing.");
            ForceLosRefresh();
        }
        ...
        private void ProbeTeleport()
        {
            try
            {
                ...
                {
                    Logger.LogWarning($"[LineOfSight] [Probe] ProbePosition is not \"x,y,z\": '{_probePosition.Value}'");
                    return;
                }

                var id = LocalClient.SelectedCreatureId;
                if (!CreaturePresenter.TryGetAsset(id, out var asset) || asset == null)
                { Logger.LogWarning("[LineOfSight] [Probe] No creature asset."); return; }

                CreatureManager.DropAtPosition(id,
                    new Unity.Mathematics.float3(px, py, pz),
                    Unity.Mathematics.quaternion.identity);
                Logger.LogInfo($"[LineOfSight] [Probe] Teleported creature to ({px:F1},{py:F1},{pz:F1})");
                ProbeRefresh();
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [Probe] {ex}"); }
        }
        ...
        private void ProbeRefresh()
        {
            try
            {
                ...
                if (CreaturePresenter.TryGetAsset(LocalClient.SelectedCreatureId, out var asset) && asset != null)
                    Logger.LogInfo($"[LineOfSight] [Probe] Creature at {asset.transform.position}");
                ForceLosRefresh();
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [Probe] {ex}"); }
        }
        ...
        private void ForceLosRefresh()
        {
            try
            {
                ...
                Logger.LogInfo($"[LineOfSight] [LoS] Forced refresh from cached view map at ({p.x:F1},{p.y:F1},{p.z:F1})");
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [LoS] ForceLosRefresh: {ex.Message}"); }
        }
        ...
        private static void EnsureFogMaskManager()
        {
            ...
            _log?.LogInfo($"[LineOfSight] FogMaskManager instance: {(_cachedFogMaskManager != null ? "found" : "NOT FOUND")}");
        }
        ...
        private static void EnsureClientModeMethods()
        {
            ...
            _btnPlayAsGMMethod = typeof(LocalClient).GetMethod("BtnPlayAsGM",
                BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
            _log?.LogInfo($"[LineOfSight] ClientMode methods: BtnPlayAsPlayer={_btnPlayAsPlayerMethod != null}, BtnPlayAsGM={_btnPlayAsGMMethod != null}");
        }
        ...
        private static void InvokeClientModeBtn(MethodInfo method)
        {
            ...
            try
            {
                ...
                _log?.LogInfo($"[LineOfSight] {method.Name} called — IsInGmMode={LocalClient.IsInGmMode}");
            }
            catch (Exception ex) { _log?.LogWarning($"[LineOfSight] {method.Name} failed: {ex.Message}"); }
        }
        ...
        private void ToggleStaticHideVolume()
        {
            ...
            {
                EnsureHvManager();
                if (_cachedHvManager == null) { Logger.LogWarning("[LineOfSight] [StaticHV] HideVolumeManager not found."); return; }

                if (_staticTestHideVolume.HasValue)
                {
                    Logger.LogInfo($"[LineOfSight] [StaticHV] Calling RemoveHideVolume id={_staticTestHideVolume.Value.Id}");
                    _cachedHvManager.RemoveHideVolume(_staticTestHideVolume.Value.Id);
                    _staticTestHideVolume = null;
                    Logger.LogInfo("[LineOfSight] [StaticHV] Removed — tiles should reappear.");
                    return;
                }
                ...
                Logger.LogInfo($"[LineOfSight] [StaticHV] Calling AddHideVolume: coord={zone.Coord} center={zone.Bounds.center} size={zone.Bounds.size}");
                _cachedHvManager.AddHideVolume(hv);
                _staticTestHideVolume = hv;
                Logger.LogInfo("[LineOfSight] [StaticHV] Added — tiles in this zone should now be hidden. Press again to remove.");
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [StaticHV] {ex}"); }
        }
        ...
        static class PatchZoneCollectDataForSector
        {
            static void Prefix(Zone __instance, out List<HideVolume> __state)
            {
                ...
                __state = ours;
                _log?.LogInfo($"[LineOfSight] [Save] Withheld {ours.Count} LoS volume(s) from zone {__instance.Coord}");
            }
        ...
        static class PatchViewCaptureReady
        {
            static void Postfix(CreatureGuid creatureId, ViewPointManager.ViewMapRef viewMapRef)
            {
                ...
                if (_viewCaptureLogCount < 12)
                {
                    ...
                    _log?.LogInfo($"[LineOfSight] ViewCapture#{_viewCaptureLogCount} creature={creatureId} "
                ...
                try
                {
                    ...
                    _log?.LogInfo($"[LineOfSight] ViewCapture ready: pos=({pos.x:F1},{pos.y:F1},{pos.z:F1}) zone={_creatureZoneCoord}");
                }
                catch (Exception ex) { _log?.LogWarning($"[LineOfSight] PatchViewCaptureReady: {ex}"); }
            }
        }
        ...
        private static void QueueFogMaskTasksForNearbyZones(Vector3 pos, ViewPointManager.ViewMapRef viewMapRef)
        {
            ...
            {
                _zoneCensusLogCount++;
                _log?.LogInfo($"[LineOfSight] ZoneCensus: tried={tried} noZone={noZone} queued={queued} range=({min})→({max})");
            }
        }
        ...
        private void Update()
        {
            ...
            if (_gmBlockPlaceKey.Value.IsDown()) ToggleLineOfSightGmBlock();
            ...
            {
                _autoStartDone = true;
                Logger.LogInfo("[LineOfSight] AutoStartTracking enabled — starting LoS tracking.");
                ToggleLosTracking();
            }
        ...
        private void LogBoardHideVolumeCensus()
        {
            ...
            {
                var board = BoardSessionManager.Board;
                if (board == null) { Logger.LogInfo("[LineOfSight] [Census] No board."); return; }

                var field = typeof(Board).GetField("_hideVolumes",
                    BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
                if (field == null) { Logger.LogWarning("[LineOfSight] [Census] Board._hideVolumes not found."); return; }
                ...
                Logger.LogInfo($"[LineOfSight] [Census] Board carries {volumes.Length} hide volume(s), {bySize.Count} distinct size(s)");
                foreach (var kv in bySize)
                    Logger.LogInfo($"[LineOfSight] [Census]   size {kv.Key} x{kv.Value}");
                for (int i = 0; i < Math.Min(5, volumes.Length); i++)
                    Logger.LogInfo($"[LineOfSight] [Census]   sample[{i}] bounds={volumes[i].Bounds} active={volumes[i].IsActive} hidePlaceables={volumes[i].HidePlaceables}");
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [Census] {ex}"); }
        }
        ...
        private static bool IsLineOfSightBlock(GmBlock block, out AtmosphereBlock atmos, out uint flags)
        {
        ...
        private static bool TryGetLineOfSightBlock(out AtmosphereBlock block, out uint flags)
        {
            block = null;
            flags = 0;
            ...
            foreach (var kv in blocks)
                if (IsLineOfSightBlock(kv.Value, out block, out flags)) return true;
            block = null;
            return false;
        }
        ...
        private void PollModeGmBlock()
        {
            ...
            else
            {
                ...
                {
                    _lastResetGeneration = gen;
                    Logger.LogInfo($"[LineOfSight] [GmBlock] Reset counter now {gen} — forgetting seen terrain.");
                    ForgetSeenTerrain();
                }
            }
            ...
            Logger.LogInfo($"[LineOfSight] [GmBlock] Board {(shouldTrack ? "requires" : "releases")} LoS mode.");
        ...
        private void RegisterRadialMenu()
        {
            ...
            // The face wraps at roughly five characters, so the name is abbreviated here and
            // spelled out in the hover title.
            var toggle = new MapMenu.ItemArgs { ValueText = "LoS", CloseMenuOnActivate = true };
            toggle.Action = (mmi, obj) =>
            {
                var block = _radialBlock;
                if (block == null) return;
                if (IsLineOfSightBlock(block, out _, out _)) ReleaseLineOfSightBlock(block);
                else AdoptLineOfSightBlock(block);
            };
            RadialUIPlugin.AddCustomButtonGMBlock("LineOfSight.Toggle", toggle, block =>
            {
                _radialBlock = block;
                bool ours = block != null && IsLineOfSightBlock(block, out _, out _);
                toggle.SubValueText = ours ? "on" : "off";
                toggle.Title = ours
                    ? "Line of sight — stop hiding what players cannot see"
                    : "Line of sight — hide what players cannot see, for everyone on this board";
                return true;
            });
            ...
            RadialUIPlugin.AddCustomButtonGMBlock("LineOfSight.Memory", memory, block =>
            {
                if (block == null || !IsLineOfSightBlock(block, out _, out var flags)) return false;
            ...
            RadialUIPlugin.AddCustomButtonGMBlock("LineOfSight.Forget", forget, block =>
            {
                if (block == null || !IsLineOfSightBlock(block, out _, out var flags)) return false;
        ...
        private void OpenForgetConfirmation()
        {
            ...
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [GmBlock] {ex}"); }
        }
        ...
        private void AdoptLineOfSightBlock(AtmosphereBlock block)
        {
            try
            {
                var board = BoardSessionManager.Board;
                if (board == null) { Logger.LogInfo("[LineOfSight] [GmBlock] No board."); return; }
                ...
                if (blocks != null)
                    foreach (var kv in new List<KeyValuePair<NGuid, GmBlock>>(blocks))
                        if (IsLineOfSightBlock(kv.Value, out var other, out _) && !other.Id.Equals(block.Id))
                ...
                Logger.LogInfo($"[LineOfSight] [GmBlock] Block {block.Id} is now the board's LineOfSight switch.");
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [GmBlock] {ex}"); }
        }

        private void ReleaseLineOfSightBlock(AtmosphereBlock block)
        {
            try
            {
                var board = BoardSessionManager.Board;
                ...
                Logger.LogInfo($"[LineOfSight] [GmBlock] Block {block.Id} released — board no longer enforces line of sight.");
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [GmBlock] {ex}"); }
        }
        ...
        private void SetGmBlockFlags(Func<uint, uint> mutate, string what)
        {
            try
            {
                var board = BoardSessionManager.Board;
                if (board == null) { Logger.LogInfo("[LineOfSight] [GmBlock] No board."); return; }
                if (!TryGetLineOfSightBlock(out var block, out var flags))
                { Logger.LogInfo("[LineOfSight] [GmBlock] No LineOfSight block on this board."); return; }

                uint next = mutate(flags);
                WriteBlockContent(board, block, MakeContent(next));
                Logger.LogInfo($"[LineOfSight] [GmBlock] {what}: flags 0x{flags:X8} -> 0x{next:X8}");
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [GmBlock] {ex}"); }
        }

        private void ToggleLineOfSightGmBlock()
        {
            try
            {
                var board = BoardSessionManager.Board;
                ...
                var blocks = GetBoardGmBlocks(board);
                if (blocks == null) { Logger.LogWarning("[LineOfSight] [GmBlock] Board._gmBlocks not found."); return; }

                foreach (var kv in new List<KeyValuePair<NGuid, GmBlock>>(blocks))
                {
                    if (!IsLineOfSightBlock(kv.Value, out _, out _)) continue;
                    board.RemoveGmBlock(kv.Key);
                    Logger.LogInfo($"[LineOfSight] [GmBlock] Removed existing LineOfSight block {kv.Key}");
                    return;
                }

                if (!CreaturePresenter.TryGetAsset(LocalClient.SelectedCreatureId, out var asset) || asset == null)
                { Logger.LogInfo("[LineOfSight] [GmBlock] No selected creature to place at."); return; }
                ...
                Logger.LogInfo($"[LineOfSight] [GmBlock] Placed LineOfSight block {block.Id} at {block.Position}");
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [GmBlock] {ex}"); }
        }
        ...
        private void LogGmBlockCensus()
        {
            ...
            {
                var board = BoardSessionManager.Board;
                if (board == null) { Logger.LogInfo("[LineOfSight] [GmCensus] No board."); return; }

                var blocks = GetBoardGmBlocks(board);
                if (blocks == null) { Logger.LogWarning("[LineOfSight] [GmCensus] Board._gmBlocks not found."); return; }

                Logger.LogInfo($"[LineOfSight] [GmCensus] Board carries {blocks.Count} GM block(s)");
                foreach (var kv in blocks)
                {
                    bool ours = IsLineOfSightBlock(kv.Value, out var atmos, out var flags);
                    string content = atmos != null ? atmos.Content.Data.ToString() : "n/a";
                    Logger.LogInfo($"[LineOfSight] [GmCensus]   {kv.Value.GetType().Name} id={kv.Key} "
                        + $"pos={kv.Value.GetPosition()} ours={ours} flags=0x{flags:X8} content={content}");
                }

                LogGmBlockPresentation();
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [GmCensus] {ex}"); }
        }
        ...
        private void LogGmBlockPresentation()
        {
            try
            {
                ...
                if (bases == null) { Logger.LogWarning("[LineOfSight] [GmVis] _blocksBases not found."); return; }
                ...
                foreach (var b in bases)
                {
                    ...
                    Logger.LogInfo($"[LineOfSight] [GmVis]   base[{n++}] name={go.name} active={go.activeInHierarchy} "
                    ...
                    foreach (var r in comp.GetComponentsInChildren<Renderer>(true))
                        Logger.LogInfo($"[LineOfSight] [GmVis]     renderer {r.name} enabled={r.enabled} "
                            + $"activeInHierarchy={r.gameObject.activeInHierarchy} layer={r.gameObject.layer}({LayerMask.LayerToName(r.gameObject.layer)})");
                }
                if (n == 0) Logger.LogInfo("[LineOfSight] [GmVis]   no presentation objects");

                var cam = Camera.main;
                if (cam != null)
                    Logger.LogInfo($"[LineOfSight] [GmVis] Camera.main={cam.name} cullingMask=0x{cam.cullingMask:X8}");
                foreach (var c in Camera.allCameras)
                    Logger.LogInfo($"[LineOfSight] [GmVis]   camera {c.name} enabled={c.enabled} cullingMask=0x{c.cullingMask:X8}");
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [GmVis] {ex}"); }
        }
        ...
        private void PurgeBoardHideVolumes()
        {
            ...
            {
                var board = BoardSessionManager.Board;
                if (board == null) { Logger.LogInfo("[LineOfSight] [Purge] No board."); return; }

                var field = typeof(Board).GetField("_hideVolumes",
                    BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
                if (field == null) { Logger.LogWarning("[LineOfSight] [Purge] Board._hideVolumes not found."); return; }
                ...
                for (int i = 0; i < volumes.Length; i++)
                {
                    var v = volumes[i];
                    ids.Add(v.Id);
                    Logger.LogInfo($"[LineOfSight] [Purge] dump[{i}] id={v.Id} center={v.Bounds.center} size={v.Bounds.size} state(active={v.IsActive},placeables={v.HidePlaceables},creatures={v.HideCreature},lights={v.HideLights}) version={v.Version}");
                }

                foreach (var id in ids) board.RemoveHideVolume(id);
                Logger.LogInfo($"[LineOfSight] [Purge] Removed {ids.Count} board hide volume(s). Bounds dumped above.");
            }
            catch (Exception ex) { Logger.LogWarning($"[LineOfSight] [Purge] {ex}"); }
        }
        ...
        private void ProcessFogResults()
        {
            ...
            {
                _rebuildLogCount++;
                _log?.LogInfo($"[LineOfSight] Rebuild: {dirty.Count} zone(s) → {boxTotal} hide box(es)");
            }
        }
        ...
        private static string FogDirectory => Path.Combine(Paths.BepInExRootPath, "LineOfSight");
        ...
        private static void LoadFogMemory()
        {
            ...
            try
            {
                ...
                using (var reader = new BinaryReader(gz))
                {
                    ...
                    {
                        _log?.LogWarning($"[LineOfSight] [Fog] {path} is not a readable LineOfSight fog file — ignoring it.");
                        return;
                    }
                ...
                _fogDirty = false;
                _log?.LogInfo($"[LineOfSight] [Fog] Restored {zones} zone(s) of seen terrain for board {_fogBoardKey}.");
            }
            catch (Exception ex)
            {
                _log?.LogWarning($"[LineOfSight] [Fog] Could not read {path}: {ex.Message}");
                lock (_zoneMasks)
                {
                    _zoneMasks.Clear();
                    _dirtyZones.Clear();
        ...
        private static void SaveFogMemory()
        {
            ...
            try
            {
                ...
                _fogDirty = false;
                _log?.LogInfo($"[LineOfSight] [Fog] Saved {snapshot.Count} zone(s) of seen terrain for board {_fogBoardKey}.");
            }
            catch (Exception ex)
            {
                _log?.LogWarning($"[LineOfSight] [Fog] Could not write {path}: {ex.Message}");
            }
        }
        ...
        private static void DeleteFogMemory()
        {
            ...
            {
                _log?.LogWarning($"[LineOfSight] [Fog] Could not delete {path}: {ex.Message}");
            }
        }
        ...
        private static void RemoveAllZoneHideVolumes()
        {
            ...
            if (removed > 0) _log?.LogInfo($"[LineOfSight] [LoS] Removed {removed} hide volumes.");
        }
